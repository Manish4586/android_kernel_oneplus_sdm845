...
{
    struct dsi_display *display = dev_get_drvdata(dev);
    int ret, hbm_los_mode;

    if (!display->panel)
	return -EINVAL;

    ret = kstrtoint(buf, 10, &hbm_los_mode);
    if (ret) {
    ...
    display->panel->hbm_los_mode = hbm_los_mode;
    if (!dsi_panel_initialized(display->panel)) {
	goto error;
    }
...
int dsi_display_get_serial_number(struct drm_connector *connector)
{
	...
	mode = panel->cur_mode;
	count = mode->priv_info->cmd_sets[
				DSI_CMD_SET_PANEL_SERIAL_NUMBER].count;
	...
	if (count) {
		...
		panel_year = 2011 + ((buf[panel->panel_year_index-1] >> 4)
					& 0x0f);
		if (panel_year == 2011)
			panel_year = 0;
